...
if (myCart.arr.length != 0) {
  ...
  getEle("cartContainner").style.display = "none";
}

renderHTMLCart(myCart.arr);
calcTotalPaid();

getEle("clearCart").onclick = function () {
  myCart.arr = [];
  renderHTMLCart(myCart.arr);
  calcTotalPaid();
  setLocalStorage(myCart.arr);
  getEle("cart-empty").style.display = "block";
  getEle("cartContainner").style.display = "none";
};

// getEle("btnPurchase").addEventListener("click", () => {
//   console.log(myCart.arr);
//   renderOrderHTML(myCart.arr);
// });
